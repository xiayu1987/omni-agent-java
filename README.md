...
请假流程举例
各公司请假规则不同，但无论流程如何变化，Agent 核心都是按照工作流执行（Graph）处理事件。
动态编排实际上只是把流程定义从代码或配置中剥离出来，变成可动态调整的“表单/节点”，本质还是工作流执行。
真正动态生成流程的需求极少，绝大多数是固定或少量可配置流程。
支持动态编排意味着：Graph 解析、流程验证、节点依赖、异常处理、版本管理等都会复杂化。
复杂度上升会带来维护成本和潜在 bug。

建议：
动态编排的收益有限，大部分场景用静态 Graph + 少量可配置节点就足够。
如有必要会加入动态编排

* 2️⃣ 多 Agent 协作现阶段的“伪需求”问题

多 Agent 协作的典型场景：一个 Agent 调用另一个 Agent。
实际效果：
提高复用（例如通用报表 Agent 可被多个业务 Agent 调用）
对功能本身没有新增价值
代码复杂度大幅增加：依赖管理、调用链、上下文传递、异常处理

建议：
如果复用价值低或流程简单 → 不用多 Agent 协作，直接在单 Agent 内完成即可。
如果复用价值高 → 可以抽象成内部工具（Tool），而不是多 Agent 调用。
如有必要会加入多Agent协作
